...
const pickProvider = (): AiProvider => {
  if (env.AI_PROVIDER === 'claude' && claudeProvider.enabled) return claudeProvider;
  if (env.AI_PROVIDER === 'gemini' && geminiProvider.enabled) return geminiProvider;
...
export const generateJson = async <T>(opts: AiGenerateOptions): Promise<T | null> => {
  try {
    const res = await aiProvider.generate({ ...opts, json: true });
