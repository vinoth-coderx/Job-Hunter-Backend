...
/**
 * Branded resume PDF renderer. Takes the seeker's structured resume
 * profile (Naukri-style sections — summary, employments, educations,
 * skills, projects, languages, accomplishments) and renders it through
 * a single-page-friendly HTML template, then prints it to PDF using
 * Puppeteer's headless Chromium.
 *
 * A dedicated browser instance is kept alive between calls so each
 * request doesn't pay the 1-2s launch tax. Auto-relaunches if the
 * underlying Chromium crashes or is disconnected.
 *
 * Output is a Buffer of the PDF bytes; the caller streams it back as
...
export const closeResumePdfBrowser = async (): Promise<void> => {
  if (browser) {
    try {
      await browser.close();
    } catch (err) {
      logger.warn(`resumePdf browser close failed: ${(err as Error).message}`);
    }
    browser = null;
  }
};
...
const renderHtml = (user: IUser): string => {
  ...
  const languagesHtml = languages
    .map(
      (l) =>
        `<span class="chip">${esc(l.language)}${l.proficiency ? ` · ${esc(l.proficiency)}` : ''}</span>`,
    )
    .join('');
...
export const generateBrandedResumePdf = async (
  user: IUser,
