import { IUser } from '../../models/User';
import { IJob } from '../../models/Job';

export interface AutoApplyScore {
  total: number;
  breakdown: {
    skills: number;
    location: number;
    experience: number;
    salary: number;
    title: number;
  };
  matchedSkills: string[];
  missingRequiredSkills: string[];
}

/**
 * Auto-Apply scorer. Implements the weighting from the product spec:
 *   skills      40%
 *   location    20%
 *   experience  20%
 *   salary      10%
 *   title       10%
 *
 * Output is 0–100. The matcher used by the home feed lives in
 * `ai/matcher.service.ts` and uses a different weighting more suited to
 * casual browsing. Auto-apply is more conservative — false positives here
 * cost users a real application, so we lean on hard skills and location.
 */
export const scoreForAutoApply = (
  user: IUser,
  job: IJob,
): AutoApplyScore => {
  const userSkills = (user.profile.skills ?? []).map((s) => s.toLowerCase());
  const jobSkills = (job.skills ?? []).map((s) => s.toLowerCase());
  const desc = (job.description ?? '').toLowerCase();
  const title = (job.title ?? '').toLowerCase();

  // Skills (40 pts)
  let skills = 0;
  let matched: string[] = [];
  if (jobSkills.length > 0) {
    matched = userSkills.filter((s) => jobSkills.includes(s));
    // Direct skill list overlap is the primary signal.
    skills = (matched.length / jobSkills.length) * 40;
    // Description bonus — small extra for skills that aren't tagged but
    // appear in the JD body. Capped so it can't dominate the score.
    const inDesc = userSkills.filter(
      (s) => !matched.includes(s) && desc.includes(s),
    ).length;
    skills += Math.min(8, inDesc * 2);
  } else if (userSkills.length > 0) {
    // Fallback when the job didn't tag skills — use the description.
    const overlap = userSkills.filter((s) => desc.includes(s));
    skills = (overlap.length / userSkills.length) * 40;
    matched = overlap;
  }
  skills = Math.min(40, skills);

  // Location (20 pts)
  let location = 0;
  const userLocs = (user.profile.preferredLocations ?? []).map((l) => l.toLowerCase());
  const jobLoc = (job.location ?? '').toLowerCase();
  if (userLocs.some((l) => jobLoc.includes(l))) {
    location = 20;
  } else if (
    job.remoteType === 'remote' &&
    user.profile.preferredRemote?.includes('remote')
  ) {
    location = 20;
  } else if (
    job.remoteType === 'hybrid' &&
    user.profile.preferredRemote?.includes('hybrid')
  ) {
    location = 12;
  } else if (userLocs.length === 0) {
    // No preference set → don't penalise this dimension.
    location = 14;
  }

  // Experience (20 pts)
  let experience = 20;
  const userYears = user.profile.experienceYears ?? 0;
  const minReq = job.experienceMinYears;
  const maxReq = job.experienceMaxYears;
  if (minReq !== undefined && maxReq !== undefined) {
    if (userYears < minReq) {
      const gap = minReq - userYears;
      experience = Math.max(0, 20 - gap * 5);
    } else if (userYears > maxReq) {
      const over = userYears - maxReq;
      // Over-qualified — small penalty (less harsh than under-qualified).
      experience = Math.max(8, 20 - over * 2);
    }
  } else if (minReq !== undefined && userYears < minReq) {
    experience = Math.max(0, 20 - (minReq - userYears) * 5);
  }

  // Salary (10 pts)
  let salary = 10;
  const expectedMin = user.profile.expectedSalaryMin;
  if (expectedMin !== undefined && expectedMin > 0) {
    const offered = job.salaryMax ?? job.salaryMin ?? 0;
    if (offered === 0) {
      // No salary disclosed — neutral.
      salary = 6;
    } else if (offered >= expectedMin) {
      salary = 10;
    } else {
      // Linear taper down to 0 at half of expected.
      salary = Math.max(0, 10 * (offered / expectedMin) - 5);
    }
  }

  // Title (10 pts) — checks preferredRoles against job title.
  let titleScore = 0;
  const userRoles = (user.profile.preferredRoles ?? []).map((r) => r.toLowerCase());
  if (userRoles.length === 0) {
    titleScore = 6; // neutral when no preference
  } else if (userRoles.some((r) => title.includes(r))) {
    titleScore = 10;
  } else if (
    userRoles.some((r) =>
      r.split(/\s+/).some((token) => token.length >= 3 && title.includes(token)),
    )
  ) {
    // Partial token match (e.g., user wants "backend developer", job title
    // has "developer" only).
    titleScore = 5;
  }

  const total = Math.round(skills + location + experience + salary + titleScore);

  // Required skills missing — used by the runner's `minSkillsMatchCount`
  // gate. We list only "required" skills (those tagged on the job) that the
  // user doesn't have.
  const missingRequiredSkills =
    jobSkills.length > 0
      ? jobSkills.filter((s) => !userSkills.includes(s))
      : [];

  return {
    total: Math.max(0, Math.min(100, total)),
    breakdown: {
      skills: Math.round(skills),
      location: Math.round(location),
      experience: Math.round(experience),
      salary: Math.round(salary),
      title: Math.round(titleScore),
    },
    matchedSkills: matched.slice(0, 12),
    missingRequiredSkills: missingRequiredSkills.slice(0, 12),
  };
};
